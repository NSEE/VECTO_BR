...
public class CombustionEngineDataProvider_v23_Tests
{
	...
	public void TestEngineDataProvider_v23_SingleFuelWHR()
	{
		...
		Assert.AreEqual(380000, inputData.RatedPowerDeclared.Value(), 1e-3);
		Assert.AreEqual(2200, inputData.RatedSpeedDeclared.AsRPM, 1e-3);
		Assert.AreEqual(WHRType.ElectricalOutput, inputData.WHRType);
		...
		Assert.AreEqual(560, mode.IdleSpeed.AsRPM);
		Assert.NotNull(mode.WasteHeatRecoveryDataElectrical);
		Assert.AreEqual(6, mode.WasteHeatRecoveryDataElectrical.GeneratedPower.Rows.Count);
		Assert.AreEqual(1.0097, mode.WasteHeatRecoveryDataElectrical.UrbanCorrectionFactor, 1e-6);
		Assert.AreEqual(1.00, mode.WasteHeatRecoveryDataElectrical.RuralCorrectionFactor, 1e-6);
		Assert.AreEqual(1.00, mode.WasteHeatRecoveryDataElectrical.MotorwayCorrectionFactor, 1e-6);
		Assert.AreEqual(1.00, mode.WasteHeatRecoveryDataElectrical.CFRegPer, 1e-6);
	...
	public void TestEngineDataProvider_v23_DualFuelWHR()
	{
		...
		Assert.AreEqual(2, mode.Fuels.Count);
		...
		var fuel1 = mode.Fuels.First();
		Assert.AreEqual(FuelType.DieselCI, fuel1.FuelType);
		Assert.AreEqual(1.0097, fuel1.WHTCUrban, 1e-5);
		Assert.AreEqual(1.0035, fuel1.WHTCRural, 1e-5);
		Assert.AreEqual(1.020, fuel1.WHTCMotorway, 1e-5);

		Assert.AreEqual(1.0, fuel1.CorrectionFactorRegPer, 1e-5);
		Assert.AreEqual(1.0, fuel1.ColdHotBalancingFactor, 1e-5);

		Assert.AreEqual(6, fuel1.FuelConsumptionMap.Rows.Count);

        var fuel2 = mode.Fuels.Skip(1).First();
        Assert.AreEqual(FuelType.NGPI, fuel2.FuelType);
		Assert.AreEqual(1.0098, fuel2.WHTCUrban, 1e-5);
		Assert.AreEqual(1.0035, fuel2.WHTCRural, 1e-5);
		Assert.AreEqual(1.020, fuel2.WHTCMotorway, 1e-5);

		Assert.AreEqual(1.0, fuel2.CorrectionFactorRegPer, 1e-5);
		Assert.AreEqual(1.0, fuel2.ColdHotBalancingFactor, 1e-5);

		Assert.AreEqual(6, fuel2.FuelConsumptionMap.Rows.Count);
	}
    ...
    private const string CombustionEngine_v23_SingleFuel = @"<?xml version=""1.0"" encoding=""UTF-8""?>
<tns:VectoInputDeclaration xmlns=""urn:tugraz:ivt:VectoAPI:DeclarationDefinitions:v2.0""
	...
	private const string CombustionEngine_v23_SingleFuel_WHR = @"<?xml version=""1.0"" encoding=""UTF-8""?>
<tns:VectoInputDeclaration xmlns=""urn:tugraz:ivt:VectoAPI:DeclarationDefinitions:v2.0""
	xmlns:tns=""urn:tugraz:ivt:VectoAPI:DeclarationComponent:v2.0""
	xmlns:v2.0=""urn:tugraz:ivt:VectoAPI:DeclarationDefinitions:v2.0""
	xmlns:v2.3=""urn:tugraz:ivt:VectoAPI:DeclarationDefinitions:v2.3""
	...
	<tns:Engine xsi:type=""v2.0:EngineComponentDeclarationType"">
					...
					<Mode>
						...
						<Fuel type=""Diesel CI"">
							...
							<FuelConsumptionMap>
								...
								<Entry engineSpeed=""560.00"" torque=""1180.00"" fuelConsumption=""1286.90""  electricPower=""200.00"" />
								<Entry engineSpeed=""2100.00"" torque=""-320.00"" fuelConsumption=""0.00""  electricPower=""200.00"" />
								<Entry engineSpeed=""2100.00"" torque=""0.00"" fuelConsumption=""1047.00""  electricPower=""200.00"" />
								<Entry engineSpeed=""2100.00"" torque=""1100.00"" fuelConsumption=""5065.00""  electricPower=""200.00"" />
							</FuelConsumptionMap>
						</Fuel>
					</Mode>
				</v2.0:Data>
				...
				</Signature>	</tns:Engine>
</tns:VectoInputDeclaration>
";

	private const string CombustionEngine_v23_DualFuel_WHR = @"<?xml version=""1.0"" encoding=""UTF-8""?>
